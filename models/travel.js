...
const travel = {
	...
	*/
	getGoods : async function(page, limit, qs) {
		try {
			...
			let sql = `SELECT COUNT(*) as cnt FROM fly_travelgoods${addWhere}`;
			const rows = await sequelize.query(sql, {
				replacements,
				type : QueryTypes.SELECT,
			});
			...
			sql = `SELECT * FROM fly_travelgoods${addWhere} ORDER BY regDt DESC LIMIT :offset, :limit`;
	...
	delete : async function(goodsCd) {
		try {
			...
			*/
			 ...
			 /** 상품 DB 데이터 삭제 S */
			 const sql = "DELETE FROM fly_travelgoods WHERE goodsCd = ?";
			 await sequelize.query(sql, {
				replacements : [goodsCd],
	...
	*/
	getPackageSchedules : async function(goodsCd) {
		try {
			const sql = "SELECT itinerary, yoils, startDate, endDate FROM fly_travelgoods WHERE goodsCd = ?";
			const rows = await sequelize.query(sql, {
				replacements : [goodsCd],
				type : QueryTypes.SELECT,
			});
